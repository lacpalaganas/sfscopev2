        ...
        <View style={styles.itemContainer}>
            <TouchableOpacity onPress={toggleExpand} style={styles.itemTouchable}>
                <Text style={styles.itemTitle}>
                    {`${item.description.trim()}\n`} <Text style={{fontSize: 11, fontStyle: 'italic', color: COLORS.gray}}>{expanded ? 'see less..' : 'see more..'}</Text>
                </Text>
            </TouchableOpacity>
            {expanded && (
                <>
                    ...
                    <Text style={styles.itemContentSubHeader}>
                        Description: <Text style={styles.itemContent}>{item.description.trim()}</Text>
                    </Text >
                    <Text style={styles.itemContentSubHeader}>
                    ...
                    <Text style={styles.itemContentSubHeader}>
                        Start Time: <Text style={styles.itemContent}>{convertTimestampToString(item.stime)}</Text>
                    </Text >
                    <Text style={styles.itemContentSubHeader}>
                        End Time: <Text style={styles.itemContent}>{convertTimestampToString(item.etime)}</Text>
                    </Text >       
