...
const RentalDetails = () => {
  const renderItem = ({ item }) => <ExpandableRentalsItem item={item} />;
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          "https://myneighborhoodscope.com/rentalApi.php"
        );
        const data = await response.json();

        var newData = data.filter(
          (item) => item.neighborhood == nhood.toString() && item.active == 1
        );
        setFilteredDataSource(newData);
        console.log(newData);
        setIsLoading(false);
  ...
  const getData = async () => {
    ...
    const result = await axios
      .get("https://myneighborhoodscope.com/rentalApi.php")
      .then((response) => {
        ...
        setFilteredDataSource(
          response.data.filter(
            (item) => item.neighborhood == nhood && item.active == 1
          )
        );
      })
      .catch((error) => {});
