...
import { Button, Text, View, SafeAreaView, ScrollView, ActivityIndicator, RefreshControl, StyleSheet, TouchableOpacity, Image, TextInput, Icon, Linking } from 'react-native'
    ...
    const filterFunction = (item) => {
        ...
        switch (item) {
            case "Most Affordable":
                ...
                newData = masterDataSource.filter((item) =>
                    item.mostAffordable === 1);
                break;
            case "Easiest Parking":
                //alert("Easisest Parking");
                newData = masterDataSource.filter((item) =>
                    item.easiestParking === 1);
